...
loader.load(
  ...
  (gltf) => {
    ...
    //
    // Pure geometric nearest-mesh lookup — no parent-chain naming involved.
    ...
    // DIAGNOSTIC ONLY — no mutation. nearestLegPart() assigns each screw to
    // whichever single leg-part mesh (arch or post) its centroid is nearest
    // to. Near the foot, left arch / right arch / middle post all converge,
    // so "nearest single mesh" can be a near-coin-flip. If a screw's
    // second-nearest part is almost as close as its assigned part, that's
    // a real candidate for misassignment — independent of (and prior to)
    // any curvature-averaging explanation.
    function nearestTwoLegParts(centroid) {
      const dists = legPartLookup.map((entry) => ({
        entry,
        d: centroid.distanceToSquared(worldCentroid(entry.mesh)),
      }));
      dists.sort((a, b) => a.d - b.d);
      return dists.slice(0, 2);
    }
    ...
    screwGroups.forEach((group) => {
      ...
      if (!info) return;
      const top2 = nearestTwoLegParts(info.c);
      ...
      if (margin < 0.03) { // tune after seeing the real distribution
        ...
        ambiguousAssignments.push({
          ...
          assignedKind: best.entry.kind,
          assignedMeshName: best.entry.mesh.name,
